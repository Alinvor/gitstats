...
class HTMLReportCreator(ReportCreator):
    def create(self, data, path):
        ...
        for file in (conf['style'], 'sortable.js', 'arrow-up.gif',
                     'arrow-down.gif', 'arrow-none.gif'):
            for base in basedirs:
                ...
                print 'Warning: "%s" not found, so not copied (searched: %s)' % (
                    file, basedirs)
        ...
        f.write(
            ...
            (data.getTotalLOC(), data.total_lines_added,
        ...
        for i in range(0, WEEKS):
            ...
            percentage = 0
            if weeks[i] in data.activity_by_year_week:
                percentage = float(data.activity_by_year_week[
        ...
        for i in range(0, 24):
            if i in hour_of_day:
                r = 127 + int((float(hour_of_day[i]) /
                               data.activity_by_hour_of_day_busiest) * 128)
                f.write('<td style="background-color: rgb(%d, 0, 0)">%d</td>' %
                        (r, hour_of_day[i]))
                fp.write('%d %d\n' % (i, hour_of_day[i]))
            else:
                f.write('<td>0</td>')
        ...
        for d in range(0, 7):
            ...
            if d in day_of_week:
                f.write('<td>%d (%.2f%%)</td>' %
                        (day_of_week[d],
                         (100.0 * day_of_week[d]) / totalcommits))
            else:
                f.write('<td>0</td>')
            f.write('</tr>')
        f.write('</table></div>')
        ...
        for weekday in range(0, 7):
            ...
            for hour in range(0, 24):
                ...
                if commits != 0:
                    ...
                    r = 127 + int(
                        (float(commits) /
                         data.activity_by_hour_of_week_busiest) * 128)
        ...
        for mm in range(1, 13):
            ...
            f.write('<tr><td>%d</td><td>%d (%.2f %%)</td></tr>' %
        ...
        for yymm in reversed(sorted(data.commits_by_month.keys())):
            f.write('<tr><td>%s</td><td>%d</td><td>%d</td><td>%d</td></tr>' %
                    (yymm, data.commits_by_month.get(
                        yymm, 0), data.lines_added_by_month.get(yymm, 0),
        ...
        for yy in reversed(sorted(data.commits_by_year.keys())):
            f.write(
                ...
                % (yy, data.commits_by_year.get(yy, 0),
                   (100.0 * data.commits_by_year.get(yy, 0)) /
                   data.getTotalCommits(), data.lines_added_by_year.get(
                       yy, 0), data.lines_removed_by_year.get(yy, 0)))
        f.write('</table></div>')
        ...
        for author in data.getAuthors(conf['max_authors']):
            ...
            f.write(
                ...
                % (author, info['commits'], info['commits_frac'],
                   info['lines_added'], info['lines_removed'],
                   info['date_first'], info['date_last'], info['timedelta'],
                   len(info['active_days']), info['place_by_commits']))
        f.write('</table>')
        ...
        if len(allauthors) > conf['max_authors']:
            f.write('<p class="moreauthors">Only top %d authors shown</p>' %
                    conf['max_authors'])
        ...
        for stamp in sorted(data.changes_by_date_by_author.keys()):
            ...
            for author in self.authors_to_plot:
                if author in data.changes_by_date_by_author[stamp].keys():
                    ...
                    commits_by_authors[
                        author] = data.changes_by_date_by_author[stamp][
                            author]['commits']
        ...
        for yymm in reversed(sorted(data.author_of_month.keys())):
            ...
            f.write(
                ...
                % (yymm, authors[0], commits,
                   ...
                   data.commits_by_month[yymm], next, len(authors)))
        ...
        for yy in reversed(sorted(data.author_of_year.keys())):
            ...
            f.write(
                '<tr><td>%s</td><td>%s</td><td>%d (%.2f%% of %d)</td><td>%s</td><td>%d</td></tr>'
                % (yy, authors[0], commits,
                   (100.0 * commits) / data.commits_by_year[yy],
                   data.commits_by_year[yy], next, len(authors)))
        f.write('</table>')
        ...
        try:
            f.write('<dt>Average file size</dt><dd>%.2f bytes</dd>' %
        ...
        for ext in sorted(data.extensions.keys()):
            ...
            f.write(
                ...
                % (ext, files, (100.0 * files) / data.getTotalFiles(), lines,
                   loc_percentage, lines / files))
        f.write('</table>')
        ...
        if len(data.tags) > 0:
            f.write('<dt>Average commits per tag</dt><dd>%.2f</dd>' %
...
plot """)
        ...
        for f in files:
            out = getpipeoutput([get_gnuplot_cmd() + ' "%s"' % f])
            if len(out) > 0:
                print out
